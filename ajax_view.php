...
if ($status_filter_param) {
    ...
    if ($status_filter_param === '1') {
        ...
        // Processing (status 4)
        $where .= " AND (items.status = '2' OR items.status = '3')";
    } elseif ($status_filter_param === '3') {
        // จ่ายแล้ว (status 5)
        $where .= " AND items.status = '5'";
    } elseif ($status_filter_param === '6') {
        // Reject (status 6)
        $where .= " AND items.status = '6'";
    } elseif ($status_filter_param === '7') {
        // ยกเลิก (status 7)
        $where .= " AND items.status = '7'";
    } else {
...
$r_total = pg_query($connections, "SELECT COUNT(*) FROM items WHERE $where");
